 ...
 *  - `ethernal` (Boolean): A flag to mention if the daemon should be resurrected after it is killed/ended (default: `false`).
 *
...
function Diable(path, exec, options) {
    ...
    var proc = Diable.daemonize(command, options);

    if (options.exit && path === process.argv[1]) {
        process.exit();
    }
